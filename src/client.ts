...
if (isBrowser) {
  ...
  log = (msg: string, ...args: any[]) => {
    ...
    const debugEnabled =
      typeof (globalThis as any).localStorage !== 'undefined' ? (globalThis as any).localStorage.getItem('DEBUG')?.includes('planllama') : false
    if (debugEnabled) {
...
export class PlanLlama extends EventEmitter {
  ...
  constructor(apiTokenOrOptions: string | PlanLlamaOptions) {
    ...
    if (typeof apiTokenOrOptions === 'string') {
      ...
      this.serverUrl = apiTokenOrOptions.serverUrl || process.env.PLANLLAMA_SERVER_URL || defaultUrl
    }
  }
  ...
  async start(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.on('connect_error', (error) => {
        clearTimeout(connectionTimeout)
        console.error('Failed to connect to PlanLlama server:', JSON.stringify(error))
   ...
   */
  async publish<T = unknown>(name: string, data: T, options?: PublishOptions): Promise<string> {
    if (!this.isStarted || !this.socket) {
      throw new Error('PlanLlama not started. Call start() first.')
    }

    return new Promise((resolve, reject) => {
      this.socket?.emit('send_job', { name, data, options }, (response: SocketResponse) => {
   ...
   * @throws {Error} If the client is not started or server returns an error.
   */
  async request<T = unknown, R = unknown>(name: string, data?: T, options?: PublishOptions): Promise<R> {
    if (!this.isStarted || !this.socket) {
      throw new Error('PlanLlama not started. Call start() first.')
    }
      ...
      this.socket?.emit('send_job', { name, data, options }, (response: SocketResponse) => {
        ...
        if (response.status === 'error') {
          ...
          const jobSuccess = (data: R) => {
            ...
            this.socket?.removeListener(`job_failed_${response.jobId}`, jobFailed)
            resolve(data)
          }
          jobFailed = (data: any) => {
            // console.log(`Job failed:`, name, data);
            this.socket?.removeListener(`job_completed_${response.jobId}`, jobSuccess)
            reject(new Error(data.error || 'Job failed'))
          }
          this.socket?.once(`job_completed_${response.jobId}`, jobSuccess)
          this.socket?.once(`job_failed_${response.jobId}`, jobFailed)
        } else {
          reject(new Error('Invalid response from server', { cause: response }))
        }
      })
    })
  }
   ...
   * @throws {Error} If the client is not started or server returns an error.
   */
  async schedule<T = unknown>(name: string, cronPattern: string, data: T, options?: ScheduleOptions): Promise<void> {
    if (!this.isStarted || !this.socket) {
      throw new Error('PlanLlama not started. Call start() first.')
    }

    return new Promise((resolve, reject) => {
      this.socket?.emit('schedule_job', { name, cronPattern, data, options }, (response: SocketResponse) => {
        if (response.status === 'error') {
          reject(new Error(response.error))
  ...
  work<T = unknown, R = unknown>(name: string, options: WorkOptions, handler: JobHandler<T, R>): void
  work<T = unknown, R = unknown>(name: string, optionsOrHandler: WorkOptions | JobHandler<T, R>, handler?: JobHandler<T, R>): void {
    if (!this.isStarted || !this.socket) {
      throw new Error('PlanLlama not started. Call start() first.')
    }
  ...
  async waitForBatch(batchId: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.socket?.emit('wait_for_batch', { batchId }, (response: SocketResponse) => {
        if (response.status === 'error') {
          reject(new Error(response.error))
        } else {
          resolve()
    ...
    return new Promise((resolve, reject) => {
      this.socket?.emit(
        ...
        (
          ...
          if (response.status === 'error') {
            reject(new Error(response.error))
          } else if (response.status === 'ok' && response.token && response.expiresAt && response.durationSeconds !== undefined) {
            resolve({
  ...
  private async fetchCurrentStepResults(jobId: string): Promise<StepResult> {
    ...
    return new Promise((resolve, reject) => {
      this.socket?.emit('fetch_step_results', { jobId }, (response: SocketResponse & { stepResults?: StepResult }) => {
        if (response.status === 'error') {
          reject(new Error(response.error))
  ...
  private async storeStepResult(jobId: string, stepName: string, result: any): Promise<void> {
    if (!this.isStarted || !this.socket) {
      throw new Error('PlanLlama not started. Call start() first.')
    }
    ...
    return new Promise((resolve, reject) => {
      this.socket?.emit('store_step_result', { jobId, stepName, result }, (response: SocketResponse) => {
        if (response.status === 'error') {
          reject(new Error(response.error))
        } else if (response.status === 'ok') {
          resolve()
  ...
  async workflow(name: string, steps: Steps): Promise<void> {
    ...
    for (const [stepName, step] of Object.entries(steps)) {
      ...
      if (typeof func !== 'function') {
        throw new Error('Step must be a function or an array ending with a function')
      }
    ...
    this.work(name, { retryLimit: 0 }, async (job) => {
      ...
      cycleCheck()
      getNextSteps(steps)
      console.log(`Starting workflow '${name}' with ${numTasks} steps. Next: [${nextSteps.join(', ')}], Pending: [${Object.keys(pendingSteps).join(', ')}]`)
      await runSteps()
      ...
      function cycleCheck() {
        ...
        for (const [stepName, step] of Object.entries(steps)) {
          if (Array.isArray(step)) {
            ...
            for (const dep of dependencies) {
              if (!steps[dep]) {
                throw new Error(`Step '${stepName}' depends on undefined step '${dep}'`)
              }
              if (!dependents[dep]) {
                dependents[dep] = []
              }
        ...
        if (processedCount !== numTasks) {
          throw new Error('workflow() cannot execute steps due to a recursive dependency')
        }
      }
      ...
      async function runStep(stepName: string) {
        const result = await self.request<StepResult, any>(`${name}/${stepName}`, stepResults)
