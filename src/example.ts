...
async function main() {
  ...
  try {
    ...
    const paymentJobId = await jobber.send('process-payment', {
      userId: 123,
      amount: 29.99,
      currency: 'USD'
    }, {
    ...
    const batchId = await jobber.sendBatch([
      { name: 'send-email', data: { to: 'user1@example.com', subject: 'Hello', body: 'Message 1' } },
      { name: 'send-email', data: { to: 'user2@example.com', subject: 'Hello', body: 'Message 2' } },
      { name: 'send-email', data: { to: 'user3@example.com', subject: 'Hello', body: 'Message 3' } }
    ]);
