...
describe('PlanLlama Error Handling and Edge Cases', () => {
  ...
  describe('Job Sending Edge Cases', () => {
    ...
    it('should handle server timeout on job sending', async () => {
      ...
      expect(mockSocket.emit).toHaveBeenCalledWith('send_job', expect.objectContaining({ name: 'timeout-job' }), expect.any(Function))
    })
    ...
    it('should handle null/undefined job data', async () => {
      ...
      await expect(planLlama.publish('test-job', null)).resolves.toBe('null-data-job')

      // Should handle undefined data
      await expect(planLlama.publish('test-job', undefined)).resolves.toBe('null-data-job')
    })
    ...
    it('should handle extremely large job payloads', async () => {
      ...
      await expect(planLlama.publish('large-job', largePayload)).resolves.toBe('large-payload-job')
    })
    ...
    it('should handle circular references in job data', async () => {
      ...
      await expect(planLlama.publish('circular-job', circularData)).resolves.toBe('circular-job')
    })
  })
  ...
  describe('Event System Edge Cases', () => {
    ...
    it('should handle event listener that throws errors', async () => {
      ...
      expect(mockSocket.emit).toHaveBeenCalledWith('job_completed', expect.any(Object))
    ...
    it('should handle memory leaks from event listeners', async () => {
      ...
      listeners.forEach((listener) => planLlama.removeListener('completed', listener))
  ...
  describe('Graceful Shutdown Edge Cases', () => {
    ...
    it('should handle multiple stop calls', async () => {
      ...
      const stopPromises = [planLlama.stop(), planLlama.stop(), planLlama.stop()]
