...
describe('Jobber Error Handling and Edge Cases', () => {
  ...
  describe('Job Sending Edge Cases', () => {
    ...
    it('should handle null/undefined job data', async () => {
      ...
      await expect(jobber.send('test-job', null)).resolves.toBe('null-data-job');
      
      // Should handle undefined data
      await expect(jobber.send('test-job', undefined)).resolves.toBe('null-data-job');
    });
    ...
    it('should handle extremely large job payloads', async () => {
      ...
      await expect(jobber.send('large-job', largePayload)).resolves.toBe('large-payload-job');
    });
    ...
    it('should handle circular references in job data', async () => {
      ...
      await expect(jobber.send('circular-job', circularData)).resolves.toBe('circular-job');
    });
  });
  ...
  describe('Event System Edge Cases', () => {
    ...
    it('should handle memory leaks from event listeners', async () => {
      ...
      expect(jobber.listenerCount('completed')).toBe(initialListenerCount + 1000);
      
      // Remove all listeners
      listeners.forEach(listener => jobber.removeListener('completed', listener));
