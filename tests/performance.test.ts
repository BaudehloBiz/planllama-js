...
describe('PlanLlama Performance Tests', () => {
  ...
  it('should handle high-volume job sending efficiently', async () => {
    ...
    const sendPromises = Array.from({ length: jobCount }, (_, index) => planLlama.publish('bulk-job', { index }))
  ...
  it('should handle memory efficiently with large job data', async () => {
    ...
    for (let i = 0; i < jobCount; i++) {
      ...
      setTimeout(() => mockSocket.mockServerEvent('work_request', job), i * 100)
    }
  ...
  it('should handle event listener performance efficiently', async () => {
    ...
    const eventsPerSecond = (eventCounts.active + eventCounts.completed + eventCounts.failed) / (duration / 1000)
