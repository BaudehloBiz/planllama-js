import type { Job, JobOptions, WorkOptions, CustomerOptions } from '../src/client';
...
describe('Jobber Constructor', () => {
  ...
  it('should throw error for invalid options', () => {
    expect(() => new Jobber({} as CustomerOptions)).toThrow('Customer options with token are required');
  });
...
describe('Jobber Job Sending', () => {
  ...
  it('should handle invalid response from server', async () => {
    ...
    await expect(jobber.send('test-job', {})).rejects.toThrow('Invalid response from server');
  });

  it('should throw error when not connected', async () => {
    await jobber.stop();
    
    await expect(jobber.send('test-job', {})).rejects.toThrow('Jobber not started. Call start() first.');
  });
});
...
describe('Jobber Job Scheduling', () => {
  ...
  it('should schedule job successfully', async () => {
    ...
    const scheduleId = await jobber.schedule('daily-report', cronPattern, jobData);
  ...
  it('should handle schedule error response', async () => {
    ...
    await expect(jobber.schedule('test-job', 'invalid', {})).rejects.toThrow('Invalid cron pattern');
  });

  it('should throw error when not connected', async () => {
    await jobber.stop();
    
    await expect(jobber.schedule('test-job', '0 9 * * *', {})).rejects.toThrow('Jobber not started. Call start() first.');
  });
});
...
describe('Jobber Job Management', () => {
  ...
  it('should handle job not found error', async () => {
    ...
    await expect(jobber.getJobById('nonexistent')).rejects.toThrow('Job not found');
  });
});
