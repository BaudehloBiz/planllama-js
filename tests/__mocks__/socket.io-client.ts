...
export class MockSocketInstance extends EventEmitter implements MockSocket {
  ...
  private isClientServerEvent(event: string): boolean {
    ...
    const clientServerEvents = [
      'send_job', 'schedule_job', 'register_worker', 'send_batch', 
      'wait_for_batch', 'get_job', 'cancel_job', 'get_queue_size',
  ...
  mockEmitWithCallback(event: string, data: unknown, callback?: (response: unknown) => void) {
